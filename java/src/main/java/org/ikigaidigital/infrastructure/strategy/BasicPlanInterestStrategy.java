...
/**
 * Interest calculation strategy for basic plan time deposits.
 * Interest is applied after minimum days requirement is met.
 */
@Component
@RequiredArgsConstructor
public class BasicPlanInterestStrategy implements InterestCalculationStrategy {

    private final TimeDepositProperties timeDepositProperties;

    @Override
    public double calculateInterest(TimeDeposit timeDeposit) {
        var planConfig = timeDepositProperties.getPlanConfig("basic");
    ...
    public boolean supports(String planType) {
        return "basic".equalsIgnoreCase(planType);
    }
}